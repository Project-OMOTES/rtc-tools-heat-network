...
def run_heat_network_optimization(heat_class, qht_class, *args, log_level=logging.INFO, **kwargs):
    """
    This function is meant to run the milp heat class and qth non-linear class sequantially.
    Both solve the same network, but the qth class is constrained with the flow direction found in the
    heat optimization. This allows to approximate the full mixed integer non-linear optimization with
    much faster computational times. Note that this does not garuantee optimality, however given that the milp heat
    optimization is reasonably close and conservative it can garuantee feasiblity.
    """
