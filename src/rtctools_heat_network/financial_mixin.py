...
class FinancialMixin(BaseComponentTypeMixin, CollocatedIntegratedOptimizationProblem):
    ...
    def __variable_operational_cost_constraints(self, ensemble_member):
        ...
        for hp in self.heat_network_components.get("heat_pump", []):
            ...
            variable_operational_cost = self.extra_variable(variable_operational_cost_var, ensemble_member)
            nominal = self.variable_nominal(variable_operational_cost_var)
            variable_operational_cost_coefficient = parameters[f"{hp}.variable_operational_cost_coefficient"]
            timesteps = np.diff(self.times())/3600

            sum = 0.0
            for i in range(1, len(self.times())):
                sum += variable_operational_cost_coefficient * elec_consumption[i] * timesteps[i-1]
            constraints.append(((variable_operational_cost - sum) /  nominal, 0.0, 0.0))
